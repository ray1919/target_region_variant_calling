...
pos_tbl <- read.xlsx("memo/POSITION_LIST.xlsx", sheet = 1)
...
for (i in 1:length(samples)) {
  ...
  vcf_fil <- VCF[paste(VCF$CHROM, ":", VCF$POS, sep = "") %in% pos_fil & VCF$INFO != "DP=0",]
...
merge_tbl <- merge(long_tbl, pos_tbl[,c("CHROM", "POS", "ID")], sort = F, all.x =T)
...
writeDataTable(wb, sheet = "PISCES RESULT", x = merge_tbl, colNames = T, rowNames = F, withFilter = F)
setColWidths(wb, sheet = "PISCES RESULT", widths = "auto", cols = 1:ncol(merge_tbl))
